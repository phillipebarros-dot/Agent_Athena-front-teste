...
export function FeedbackActions({ message, onSendFeedback, onToggleFb, onRegenerate, onTtsStateChange }: FeedbackPanelProps & { fbOpen: boolean; onToggleFb: () => void; onRegenerate?: () => void; onTtsStateChange?: (active: boolean) => void }) {
  ...
  return (
    <div style={css('display:flex; align-items:center; gap:4px; margin-top:8px')}>
      ...
      <B t="button" title="Ouvir" onClick={async () => { if (ttsLoading) return; setTtsLoading(true); setTtsError(false); onTtsStateChange?.(true); const ok = await playTts(message.content); if (!ok) setTtsError(true); setTtsLoading(false); onTtsStateChange?.(false); }} c="padding:4px 9px; border-radius:6px; border:1px solid var(--border); background:transparent; color:var(--muted); font-family:var(--font-body); font-size:11px; cursor:pointer; display:inline-flex; align-items:center; gap:5px" h="border-color:var(--red-dim); color:var(--white)">
        <IC s={12} d='<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/>' w={2} />{ttsLoading ? 'Gerando…' : ttsError ? 'Falhou' : 'Ouvir'}
      </B>
...
// TTS helper — returns true on success, false on failure
