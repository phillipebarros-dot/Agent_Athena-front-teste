...
export function FeedbackActions({ message, onSendFeedback, onToggleFb }: FeedbackPanelProps & { fbOpen: boolean; onToggleFb: () => void }) {
  return (
    <div style={css('display:flex; align-items:center; gap:4px; margin-top:8px')}>
      ...
      <B t="button" title="Copiar" onClick={() => navigator.clipboard?.writeText(message.content)} c="padding:4px 9px; border-radius:6px; border:1px solid var(--border); background:transparent; color:var(--muted); font-family:var(--font-body); font-size:11px; cursor:pointer; display:inline-flex; align-items:center; gap:5px" h="border-color:var(--red-dim); color:var(--white)">
